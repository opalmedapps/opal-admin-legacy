...
controller('hospitalMap', function ($scope, $filter, $sce, $state, $uibModal, hospitalMapCollectionService, Session) {
	...
	var cellTemplateOperations = '<div style="text-align:center; padding-top: 5px;">' +
		'<strong><a href="" ng-click="grid.appScope.editHosMap(row.entity)"><i title="'+$filter('translate')('HOSPITAL_MAPS.LIST.EDIT')+'" class="fa fa-pencil" aria-hidden="true"></i></a></strong> ' +
		'- <strong><a href="" ng-click="grid.appScope.deleteHosMap(row.entity)"><i title="'+$filter('translate')('HOSPITAL_MAPS.LIST.DELETE')+'" class="fa fa-trash" aria-hidden="true"></i></a></strong></div>';
	...
	function getHospitalMapsList() {
		hospitalMapCollectionService.getHospitalMaps().then(function (response) {
			...
			alert($filter('translate')('HOSPITAL_MAPS.LIST.ERROR_LIST') + "\r\n\r\n" + response.status + " " + response.data);
		});
	}
		...
		modalInstance.closed.then(function () {
			...
			if (!$scope.updatedHosMap) {
				hospitalMapCollectionService.generateQRCode($scope.currentHosMap.qrid, $scope.oldqrid).then(function () {
					...
					alert($filter('translate')('HOSPITAL_MAPS.LIST.ERROR_QR') +"\n\r\n\r" + response.status + " - " + response.data);
				});
			}

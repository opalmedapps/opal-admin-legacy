angular.module('opalAdmin.controllers.customCodes', ['ngAnimate', 'ui.bootstrap', 'ui.grid', 'ui.grid.resizeColumns']).


/******************************************************************************
 * Diagnosis Translation Page controller
 *******************************************************************************/
controller('customCodes', function ($scope, $filter, $uibModal, diagnosisCollectionService, educationalMaterialCollectionService, uiGridConstants, $state, Session) {
	...
	var cellTemplateOperations = '<div style="text-align:center; padding-top: 5px;">' +
		'<strong><a href="" ng-click="grid.appScope.editDiagnosisTranslation(row.entity)"><i title="' + $filter('translate')('DIAGNOSIS.LIST.EDIT') + '" class="fa fa-pencil" aria-hidden="true"></i></a></strong> ' +
		'- <strong><a href="" ng-click="grid.appScope.deleteDiagnosisTranslation(row.entity)"><i title="' + $filter('translate')('DIAGNOSIS.LIST.DELETE') + '" class="fa fa-trash" aria-hidden="true"></i></a></strong></div>';
	...
	function getDiagnosisTranslationList() {
		diagnosisCollectionService.getDiagnosisTranslations().then(function (response) {
			...
			alert($filter('translate')('ALIAS.LIST.ERROR_LIST') + "\r\n\r\n" + response.status + " - " + response.data);
		});
	}
});

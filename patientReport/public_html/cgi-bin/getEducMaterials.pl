...
# Script to find list of educational materials in Opal Dev, given one type of material to search for
...
my $sql1="
	...
	WHERE
		EducationalMaterialType_EN = '$type'";
...
foreach my $r (@$data1){
	...
	$newRec = newList rec(split(',',substr($nextRow,1,-1)));
	my $key = (keys %$newRec)[0];
	$types_json_data->{$key} ||= [];
	push @{$types_json_data->{$key}}, $newRec->{$key};
}

my $JSON = $json_str->encode($types_json_data); #Encode the diagnosis JSON (And any of the other JSONs that have been appended to diagnosis)
print "$JSON\n"; #Return
$query1->finish;
$dbh->disconnect;
exit;

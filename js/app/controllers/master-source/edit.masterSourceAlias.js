angular.module('opalAdmin.controllers.masterSourceAlias.edit', ['ngAnimate', 'ui.bootstrap', 'ui.grid', 'ui.grid.resizeColumns']).

controller('masterSourceAlias.edit', function ($scope, $filter, $uibModal, $uibModalInstance, masterSourceCollectionService, Session, ErrorHandler) {
	$scope.oldData = {};
	...
	var arrValidationUpdate = [
		$filter('translate')('MASTER_SOURCE_MODULE.ALIAS_EDIT.VALIDATION_SOURCE'),
		$filter('translate')('MASTER_SOURCE_MODULE.ALIAS_EDIT.VALIDATION_EXTERNAL_ID'),
		$filter('translate')('MASTER_SOURCE_MODULE.ALIAS_EDIT.VALIDATION_CODE'),
		$filter('translate')('MASTER_SOURCE_MODULE.ALIAS_EDIT.VALIDATION_DESCRIPTION')
	];

	var arrValidationGetDetails = [
		$filter('translate')('MASTER_SOURCE_MODULE.ALIAS_EDIT.VALIDATION_SOURCE'),
		$filter('translate')('MASTER_SOURCE_MODULE.ALIAS_EDIT.VALIDATION_EXTERNAL_ID'),
		$filter('translate')('MASTER_SOURCE_MODULE.ALIAS_EDIT.NOT_FOUND')
	];
	...
	// Call our API service to get the current alias translation details
	masterSourceCollectionService.getAliasDetails($scope.currentAlias.externalId, $scope.currentAlias.source).then(function (response) {
		...
		ErrorHandler.onError(err, $filter('translate')('MASTER_SOURCE_MODULE.ALIAS_EDIT.ERROR_DETAILS'), arrValidationGetDetails);
	...
	$scope.updateCustomCode = function() {
		if($scope.formReady && $scope.changesDetected) {
			var ready = {};
			ready[0] = {
				externalId: $scope.currentAlias.externalId,
				source: $scope.currentAlias.source,
			...
			$.ajax({
				...
				error: function (err) {
					...
					ErrorHandler.onError(err, $filter('translate')('MASTER_SOURCE_MODULE.ALIAS_EDIT.ERROR_UPDATE'), arrValidationUpdate);
				},
				complete: function () {
					$uibModalInstance.close();
				}

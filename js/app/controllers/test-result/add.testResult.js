...
/*	$scope.removeAdditionalLink = function (index) {
	...
	$scope.submitTestResult = function () {
		if ($scope.checkForm()) {
			...
			$scope.newTestResult.user = Session.retrieveObject('user');
	...
	$scope.searchEduMatsFilter = function (edumat) {
		...
		return !$scope.eduMatFilter || keyword.test(Session.retrieveObject('user').language.toUpperCase() === "FR"?edumat.name_FR:edumat.name_EN);
	};

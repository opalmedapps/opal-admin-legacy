angular.module('opalAdmin.controllers.publication.add', ['ngAnimate', 'ui.bootstrap', 'ui.grid', 'ui.bootstrap.materialPicker', 'textAngular', 'multipleDatePicker', 'angularjs-dropdown-multiselect']).
	...
	controller('publication.add', function ($scope, $filter, $uibModal, $state, $locale, publicationCollectionService, Session, filterCollectionService, FrequencyFilterService, filterFilter ) {
		...
		$scope.resetPublishDate = function() {
			$scope.toSubmit.publishDateTime.publish_date = null;
			$scope.toSubmit.publishDateTime.publish_time = null;
			$scope.publishDateUpdate();
		}
		...
		$scope.changeTriggers = function(triggerList) {
			...
			angular.forEach(triggerList, function (item) {
				...
				if(item.added) {
					if (pos == -1) {
						console.log("not found and pushed");

...
define("OPAL_GET_EDUCATIONAL_MATERIAL","
    SELECT DISTINCT em.EducationalMaterialControlSerNum AS serial, em.EducationalMaterialType_EN AS type_EN, em.EducationalMaterialType_FR AS type_FR, em.Name_EN AS name_EN, em.Name_FR AS name_FR, em.EducationalMaterialCategoryId AS purpose_ID, em.URL_EN AS url_EN, em.URL_FR AS url_FR, em.PublishFlag AS publish, em.ParentFlag AS parentFlag, em.ShareURL_EN AS share_url_EN, em.ShareURL_FR AS share_url_FR, em.LastUpdated AS lastupdated, (SELECT COUNT(*) AS locked FROM ".OPAL_FILTERS_TABLE." f WHERE f.ControlTableSerNum = em.EducationalMaterialControlSerNum and ControlTable = '".OPAL_EDUCATION_MATERIAL_CONTROL_TABLE."') AS locked, (case WHEN em.ParentFlag = 1 then (SELECT COALESCE(round(AVG(emr.RatingValue)), 0) FROM EducationalMaterialRating emr WHERE emr.EducationalMaterialControlSerNum = em.EducationalMaterialControlSerNum) ELSE 0 END) AS rating FROM ".OPAL_EDUCATION_MATERIAL_CONTROL_TABLE." em WHERE em.deleted = 0;
");
...
define("OPAL_GET_TEST_RESULTS","
    ...
    Group_EN AS group_EN, Group_FR AS group_FR, 0 AS changed, InterpretationRecommended AS interpretability FROM ".OPAL_TEST_CONTROL_TABLE.";
");
...
define("OPAL_GET_TEST_RESULT_DETAILS","
    ...
    Group_EN AS group_EN, Group_FR AS group_FR, EducationalMaterialControlSerNum AS eduMatSer, InterpretationRecommended AS interpretability FROM ".OPAL_TEST_CONTROL_TABLE."
...
define("OPAL_UPDATE_TEST_CONTROL", "
    ...
    SessionId = :SessionId, InterpretationRecommended = :InterpretationRecommended WHERE TestControlSerNum = :TestControlSerNum;"
);

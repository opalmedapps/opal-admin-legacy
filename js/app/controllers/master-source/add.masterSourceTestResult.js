angular.module('opalAdmin.controllers.masterSourceTestResult.add', ['ngAnimate', 'ui.bootstrap']).
controller('masterSourceTestResult.add', function ($scope, $filter, $uibModal, masterSourceCollectionService, $state, Session, ErrorHandler) {
	...
	var arrValidationInsert = [
		$filter('translate')('MASTER_SOURCE_MODULE.TEST_RESULT_ADD.VALIDATION_SOURCE'),
		$filter('translate')('MASTER_SOURCE_MODULE.TEST_RESULT_ADD.VALIDATION_EXTERNAL_ID'),
		$filter('translate')('MASTER_SOURCE_MODULE.TEST_RESULT_ADD.VALIDATION_CODE'),
		$filter('translate')('MASTER_SOURCE_MODULE.TEST_RESULT_ADD.VALIDATION_DESCRIPTION'),
		$filter('translate')('MASTER_SOURCE_MODULE.TEST_RESULT_ADD.VALIDATION_DATE'),
	];
	...
	// Call our API to ge the list of test results
	masterSourceCollectionService.getExternalSourceDatabase().then(function (response) {
		$scope.dbList = response.data; // Assign value
	}).catch(function(err) {
		ErrorHandler.onError(err, $filter('translate')('MASTER_SOURCE_MODULE.TEST_RESULT_ADD.ERROR_SOURCE'));
		$state.go('master-source/test-result');
	});
	...
	$scope.submitCustomCode = function () {
		...
		masterSourceCollectionService.isMasterSourceTestResultExists($scope.toSubmit.sourceDatabaseId.value, $scope.toSubmit.details.code).then(function (response) {
			var resultServer = response.data;
			if(response.data.code !== undefined) {
				var warningMsg = $filter('translate')('MASTER_SOURCE_MODULE.TEST_RESULT_ADD.ALREADY_EXISTS');
				warningMsg = warningMsg.replace("%%CODE%%", response.data.code);
				warningMsg = warningMsg.replace("%%DESCRIPTION%%", response.data.description);
				warningMsg = warningMsg.replace("%%STATUS%%", (response.data.deleted === "0" ? $filter('translate')('MASTER_SOURCE_MODULE.TEST_RESULT_ADD.ACTIVE') : $filter('translate')('MASTER_SOURCE_MODULE.TEST_RESULT_ADD.DELETED') ));
				var anwser = confirm(warningMsg);
				if(anwser == true) {
					submitTestResultAjax(ready);
				}
			} else
				submitTestResultAjax(ready);
		}).catch(function(err) {
			err.responseText = err.data;
			ErrorHandler.onError(err, $filter('translate')('MASTER_SOURCE_MODULE.TEST_RESULT_ADD.VALIDATE_ERROR'), arrValidationExists);
			$state.go('master-source/test-result');
		});
	};

	function submitTestResultAjax(ready) {
		$.ajax({
			type: 'POST',
			url: 'master-source/insert/test-results',
			data: ready,
			success: function () {},
			error: function (err) {
				err.responseText = JSON.parse(err.responseText)[0];
				ErrorHandler.onError(err, $filter('translate')('MASTER_SOURCE_MODULE.TEST_RESULT_ADD.ERROR_ADD'), arrValidationInsert);
			},
			complete: function () {
				$state.go('master-source/test-result');
			}
